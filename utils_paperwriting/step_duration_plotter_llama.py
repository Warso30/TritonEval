...
N_zoom = 700
# zoom_df = df[columns==["stepwise", "epsilon"]][df["steps"] >= df["steps"].max() - N_zoom]
zoom_df = df.loc[
    df["type_name"].isin(["Stepwise", "Epsilon"])
    & (df["steps"] >= df["steps"].max() - N_zoom)
]
axins = inset_axes(ax, width="80%", height="30%", loc="upper right", borderpad=0.8)
sns.lineplot(
    data=zoom_df,
    x="steps",
    y="time",
...
ax.set_ylabel("Time(ms) per Token (log scale)")
...
plt.tight_layout()
plt.savefig(
    f"plots/{args.MODEL_NAME}_step_time_promptlen{args.promptlen}.png",
    dpi=600,
    bbox_inches="tight",
)
plt.savefig(
    f"plots/{args.MODEL_NAME}_step_time_promptlen{args.promptlen}.svg",
    format="svg",
    bbox_inches="tight",
)
plt.close(fig)
