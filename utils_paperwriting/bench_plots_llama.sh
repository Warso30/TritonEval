# #!/bin/bash

# folder="/Users/kangqiwang/Desktop/various/TritonEval/stats/transformer"

# # 提取模型名（第一个下划线前）
# models=$(ls ${folder}/*.json | xargs -n1 basename | sed -E 's/^([^_]*)_.*$/\1/' | sort | uniq)

# model="t5-small"  # Specify the model you want to run
# default_file=$(ls ${folder}/${model}_default_*.json 2>/dev/null | head -n 1)
# epsilon_file=$(ls ${folder}/${model}_epsilon_*.json 2>/dev/null | head -n 1)
# stepwise_file=$(ls ${folder}/${model}_stepwise_*.json 2>/dev/null | head -n 1)

# if [[ -n "$default_file" && -n "$epsilon_file" && -n "$stepwise_file" ]]; then
#     echo "Running for model: $model"
#     python step_duration_plotter_t5.py \
#         --MODEL_NAME "$model" \
#         --default "$default_file" \
#         --epsilon "$epsilon_file" \
#         --stepwise "$stepwise_file"
# else
#     echo "Skipping $model: missing one of default/epsilon/stepwise"
# fi

#!/usr/bin/env bash

# 统计文件所在目录
STAT_DIR="/Users/kangqiwang/Desktop/various/TritonEval/stats/stats/llama-1000tokens"

# 你的 Python 脚本路径（假设和这个脚本在同一目录）
PY_SCRIPT="step_duration_plotter_llama.py"

for LEN in 64 128 256 512; do
    echo "=== 处理 promptlength=${LEN} ==="

    # 找到对应的 JSON 文件
    JSON_DEFAULT=$(ls "${STAT_DIR}"/llama_default_promptlength${LEN}_*.json)
    JSON_EPSILON=$(ls "${STAT_DIR}"/llama_epsilon_promptlength${LEN}_*.json)
    JSON_STEPWISE=$(ls "${STAT_DIR}"/llama_stepwise_promptlength${LEN}_*.json)

    if [[ -n "$JSON_DEFAULT" && -n "$JSON_EPSILON" && -n "$JSON_STEPWISE" ]]; then
    python step_duration_plotter_llama.py \
        --MODEL_NAME "llama" \
        --default "$JSON_DEFAULT" \
        --epsilon "$JSON_EPSILON" \
        --stepwise "$JSON_STEPWISE" \
        --promptlen "$LEN"
    fi

    echo "完成 promptlength=${LEN}"
done

echo "所有任务已完成。"

